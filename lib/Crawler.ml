...
(* TODO: Make a functor out of this later for the most general cases *)
module type INTROSPECT = sig
  val query_args: Machine.t -> string
end

module StandardOut: INTROSPECT = struct
...
module Tabulation: INTROSPECT = struct
  open Ascii_table
    ...
    | Structure address -> (
      ...
      | Functor (name, arity) ->
         ...
         let folder (acc: string list) elem: string list =
           ...
           if acc = [] then inner else acc@inner
         in
         let middle: string list = List.fold_left folder [] (List.of_enum (0 --^ arity)) in
         if arity = 0 then [name] else name::middle
      | _ -> failwith "Nonsense via structure")
    | Reference _ -> (
      let next = Evaluator.deref_cell register store in
  ...
  let query_args ({ store; query_variables; _ } : Machine.t) (*: (string, string list) BatMap.t*) =
    let folder (acc: (string, string list) BatMap.t) (variable, cell): (string,string list) BatMap.t =
      BatMap.add variable (inspect store cell) acc
    in
    let vars = BatMap.to_seq @@ BatSeq.fold_left folder BatMap.empty (BatMap.to_seq query_variables) in
    let inner: string Column.t list = [Column.create "" Fun.id ] in
    let columns: (string * string list) Column.t list =
      List.of_seq @@ Seq.map (fun (name, _) ->
          Column.create name (fun (_, (content: string list)) ->
              Ascii_table.to_string_noattr ~bars:`Unicode inner content)) vars in
    Ascii_table.to_string ~header_attr:[`Magenta] ~bars:`Unicode columns @@ List.of_seq vars
end

(* Curse upon your house, Lemos *)
(* TODO: Please burn this thing down asap *)
let rec inspect_list (store : Cell.t Store.t) (address : int) : string =
  let head = inspect store (Reference address) in
  let tail =
    match Evaluator.deref_cell (Reference (address + 1)) store with

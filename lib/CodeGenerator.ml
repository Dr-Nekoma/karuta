...
let rec cell_register : RegisterAllocator.register -> Cell.register = function
...
and emit_queue_nested_fact_argument
...
and emit_queue_fact_arguments
  ...
  | Some (rest, d) ->
      ...
      let result =
        emit_queue_nested_fact_argument (generator, allocator, store) d
      in
      emit_queue_fact_arguments result

and emit_fact_argument
    ((({ variables; _ } as generator), ({ registers; _ } as allocator), store) :
...
and emit_query_argument
  ...
  | Functor { namef; elements; arity } ->
      ...
      let generator, store = add_instruction (generator, store) instruction in
      List.fold_left emit_query_argument (generator, allocator, store) elements
...
and allocate_head ({ elements; _ } : Ast.func)
...
and generate
  ...
  | Query { namef; elements; arity } ->
      let generator, allocator, store =
        List.fold_left emit_query_argument
          (generator, allocator, store)
          elements
      in
      let instruction = Cell.Call (namef, arity) in
  ...
  | Declaration { head = { elements; namef; arity }; body = [] } ->
      ...
      in
      let generator, allocator, store =
        emit_queue_fact_arguments (generator, allocator, store)
      in

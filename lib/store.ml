...
module Make (Layout : Layout) = struct
  ...
  let heap_start = 0
  let heap_get (mem: 'a t) (index: int): 'a = limited_get heap_start Layout.heap_size mem index
  let heap_put (elem: 'a) (index: int) (mem: 'a t): 'a t = limited_put heap_start Layout.heap_size elem index mem
  ...
  let pdl_push (elem: 'a) (mem: 'a t): 'a t =
    if !pdl_tracker < trail_start
    then failwith "Stack is full!"
    else
